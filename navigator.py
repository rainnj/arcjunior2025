...
def run_live():
    ...
    initial_position = (packet.lat, packet.lon)

    # Nearby points to navigate to (~15m radius from starting point)
    target_waypoints = [
        (52.474600, 13.457250),
        (52.474645, 13.457280),
        (52.474685, 13.457240),
    ...
    # Waypoints list starts from current position
    waypoints = [initial_position] + target_waypoints
    ...
    while waypoint_index < len(waypoints):
        try:
            ...
            if prev_lat is not None:
                ...
                if distance < 2.0:
                    print(f"✅ Reached waypoint {waypoint_index}/{len(waypoints) - 1}")
                    waypoint_index += 1
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--simulate":
        simulated_path = load_gps_waypoints('gpslocations/simulated-path.txt')  # Optional
        waypoints = load_gps_waypoints('gpslocations/sample-gpslocations.txt')  # Optional
